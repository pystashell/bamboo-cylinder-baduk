...
> 文档版本：1.4
> 代码快照：`codex/大厅版` 当前工作区，包含 setup-first 在线房间、持久多轮对局、隐藏大厅单向索引、矩形棋盘、SGF、AI 自对弈、聊天、权威计时、观战分析和版本化发布等增量；最终测试与线上状态仍以第 20、21 节的实测证据为准
...
1. 打开根地址时，先按浏览器语言或已保存偏好显示中文/英文，并规范到 `/single`。这里是完全本地的房间设置阶段：默认只展示 `19 × 19` 竹筒棋盘的 120° 弧面预览，不自动开始棋局，也不连接服务器。
2. 单人页和在线房间使用同一套棋盘、五标签侧栏、设置表单和动作处理器。用户可在 `/single` 本地下棋，也可创建/加入 `/online/{ROOM_CODE}`；切换 transport 不得重新实现一套 UI。
3. 在线房间创建后固定先进入 `setup`，不自动开局。房主在同一“棋盘设置”中选择拓扑、宽高、规则、贴目、计时以及好友、人机、AI 对 AI或同机双人模式，再发起当前轮对局。
4. 好友模式进入 `invited`，被邀请方必须明确接受或拒绝；接受后才进入 `playing` 并启动权威时钟。人机、AI 对 AI 和同机双人由当前房主浏览器控制对应颜色，确认设置后可以直接开始，但整盘仍同步到服务器并开放观战。
5. 实际建立棋局后，侧栏自动回到“棋局”；通过 OGS 风格的同一组标签在棋局、AI 分析、聊天、棋谱和“棋盘设置”间切换，棋盘始终占据主视觉区。
6. 随时切换可用视图、滑动展开面、旋转三维曲面、缩放和关闭音效，并从黑白时钟卡片看到当前用时。
7. 双方停着后标记死子并点目；联机时双方确认结果；时间耗尽则按超时终局。
8. 使用悔棋处理最近失误，必要时二次确认后认输；或进入完整复盘逐手播放、切换视图并查看多个 AI 候选及后续变化。
9. 导出 SGF，或导入 SGF 后以任意可用视图播放和 AI 分析；超时和认输结果分别兼容标准 `B+T/W+T` 与 `B+R/W+R`。
10. 联机时用 Unicode 文字、Emoji、表情包和可点击棋点坐标沟通，或选择“仅观战”进入房间并在自己的浏览器中做不影响棋手的局势分析。
11. 实验性在线大厅只对现有房间做可丢弃的只读索引；用户即使不知道大厅入口，仍能通过房间码或分享链接完成全部在线流程。

### 1.3 明确不在当前范围

- 球面棋盘、正方体棋盘或任意曲面编辑器。
- 常规有四个角的平面棋盘作为新局形状选项。
- 账号体系、随机匹配、排行榜、等级分、支付或云端棋谱库。大厅当前仅为不公开入口的实验性房间索引，不承诺账号、匹配或赛事能力。
...
- 宽度 25 时最后一列为 `Z`；宽度 30 时继续使用 `AA` 至 `AE`，并保持跳过 `I` 的围棋坐标习惯。
- 聊天坐标和界面坐标都使用这套规则。
...
- 按 FF[4] 使用 `a-zA-Z`，不跳过 `i`；宽或高超过 26 时使用大写坐标字符，因此 `30 × 20` 仍是标准可解析的 `SZ[30:20]`。
...
| --- | ---: | ---: | ---: | --- |
| `width` | 19 | 5–30 | 1 | 四舍五入后夹到范围；空值回退 19 |
| `height` | 19 | 5–30 | 1 | 四舍五入后夹到范围；空值回退 19 |
...
这些值是大厅预览和下一盘邀请的默认草稿；`currentPlayer = black` 只规定真正开局后的先手，不表示页面加载时已有棋局。初次进入不得因此启动 AI、计时器、Replay、WebSocket、房间创建请求或可落子的 live session。

### 3.3 建立新局

- 首盘只能由大厅的“邀请对局 / Invite Game”流程建立；“建立新局”是已有棋局中的后续动作，不能在首次页面加载时被隐式调用。
- 有棋局进度时，建立新局必须弹出确认，并显示目标拓扑、`width × height` 和“当前进度会被清除”。
- 一局结束后统一显示“直接进行下一局 / Start Next Game Now”和“调整下一局设置 / Adjust Next-Game Settings”两项终局入口，不在终局操作区另设“退出房间”。“直接进行下一局”按上一局完全相同的拓扑、宽高、规则、贴目、计时、controller 与对手配置进入同一 `request_game` 管线：非好友模式可立即建立新轮，好友模式仍须产生新 invitation 并由对方明确接受；“调整下一局设置”只进入预填上一局参数的设置确认阶段，不得立即建立棋局，只有点击“使用当前设置开始下一局”后才提交该管线。
- `transport=local` 时，设置确认阶段必须保留上一局的 controller 与对手上下文，包括本地双人、AI 模型、玩家执子颜色和 AI 自对弈模式；`transport=online` 时仅黑方房主可以编辑并确认下一局，房间号、成员、观众、聊天记录以及最近 24 轮归档全部保留，并用新的 `match.controllers` 替换本轮控制关系。控制器是人类还是 AI 不产生第二套设置或新局流程。
- 房间信息区原有的退出动作继续负责释放在线席位或结束当前对手上下文，但它不是终局两项新局操作之一，也不得替代“调整下一局设置”。
...
- 两方都是在线真人时，必须由两个已认证 controller operator 分别确认。本地或在线同设备对弈由该设备代表它拥有的两个 controller 确认；人与 AI 时，AI 不独立判断异形棋盘死活，人类确认后 session 可代表其操作的 AI controller 补齐确认，因此表现为“直接确认”。
...
所有 controller 组合都复用同一 match session。`local` 支持 `human/human`、`human/ai`、`ai/human`、`ai/ai`；v2 online 以 `friend`（两个 human operator）、`human-ai`（房主操作 human 与 AI）、`ai-ai`（房主操作两个 AI）和 `local`（房主操作两个 human）四种持久模式表达当前组合。在线 AI 始终由拥有相应 controller 的浏览器运行，服务端只验证命令并同步权威状态。`online-spectator` 是只读访问身份，不是第五种规则模式；`replay` 是覆盖 live session 的只读展示层，也不是 transport 或 controller。

UI 只调用统一业务意图，例如 `play`、`pass`、`toggle_dead`、`finish_scoring`、`resume_play`、`undo`、`resign` 和 `new_game`。`local` adapter 同步调用本地引擎；`online` adapter 将意图路由为协议允许的明确命令，例如真人 `play/pass`、AI controller 的 `ai_play/ai_pass`、人机直接悔棋 `direct_undo_ai_round`、同机双人直接悔棋 `direct_undo_local_round`。命令名可以反映不同权限边界，但不得产生第二套规则或棋盘。任何组件都不得用“当前模式”直接改棋盘，或维护一份本地/AI/联机专属棋局副本。
...
页面外壳必须在围棋引擎的 `play/scoring/finished` 之外，维护一层清楚的产品生命周期：

~~~ts
type MatchLifecycle = "setup" | "invited" | "playing" | "finished";
~~~

- `/single` 首次加载和在线房间新建后固定进入 `setup`。此时不存在可操作的 live round；渲染器可以用下一盘设置草稿生成棋盘预览，但该预览不拥有手数、Replay、AI 任务或权威计时，也不得被序列化成一盘已开始的棋。
- 设置阶段的主要动作是“邀请对局 / Invite Game”或“使用当前设置开始下一局”，不得在页面加载或房间创建成功时隐式开局。
- 在线 `request_game` 必须包含完整且经过校验的轮次设置与 `mode=friend|human-ai|ai-ai|local`。好友模式产生带 revision 的 `invited` 请求；被邀请方可 `respond_game(accept|reject)`，房主可 `cancel_game_request`。拒绝或取消后回到 `setup`，不修改上一轮归档。
- 非好友模式由房主浏览器承担所选双方 controller，确认后可从 `setup/finished` 直接进入 `playing`；它们与好友模式使用同一房间、RoundEngine、设置 DOM 和观战通道。
- `setup` 和 `invited` 中，棋盘仍可切换视图、旋转、缩放和平移，用于理解目标拓扑；任何左键/轻点都不能落子，也不能显示可落子幽灵棋、播放落子音或推进状态。停一手、悔棋、认输、点目、复盘播放、AI 分析及棋局计时必须隐藏或明确禁用。
- 只有进入 `playing` 后，统一 match session 才解锁规则动作、AI 调度、计时和 Replay。终局后进入 `finished`；再次邀请或建立新局仍经过同一配置与确认管线。
- 本地和在线不得各自实现一套大厅、邀请弹层或棋局组件。共同流程先产生经校验的 `MatchConfig`，然后只在最后一步选择 local/online transport adapter；controller 选择、设置 DOM、棋盘、侧栏、动作意图和状态文案均共享。
- 页面进入 `/single` 不得自动请求房间 API、建立 WebSocket、加载 AI 模型或启动 Worker/计时器。进入 `/online/{code}` 才恢复/加入对应房间；进入实验性大厅只读取派生索引，不得创建或改变任何房间。

## 9. Match session 组合详细要求

### 9.1 本地 transport

- 邀请草稿默认 `transport=local`，不需要登录或网络；默认双方 controller 都是 `human`，同一设备按黑白顺序操作。这里的默认值不自动建立棋局，必须等用户在邀请弹层最终确认。
...
- controller 的改变随新轮设置生效。online 由黑方房主在 `request_game` 中一次性声明双方 controller；DO 将其持久化到 `match.controllers` 并广播，不能在客户端无提示地改 controller。旧 `attach_ai/detach_ai` 只作为早期房间兼容协议，不是 v2 setup-first 房间的主要入口。
...
- AI 自对弈可在 `local` 和 `online` transport 运行。online 时两个 AI controller 都归当前房主浏览器，棋步逐手提交给 DO 验证并广播；AI 分析本身仍不进入服务器。

### 9.4 在线 transport、持久 controller 与观战

- 普通真人房中，创建者是房主并获得黑方身份，第二位真人可加入房间但不会触发自动开局；好友邀请被其明确接受后，DO 才把双方 controller 固定到本轮。后来者成为旁观者。加入弹层必须另有“仅观战”，即使有空席也不能擅自改成玩家。
- v2 snapshot 顶层 `match.controllers.black/white` 是本轮 controller 权威来源，`members` 只表达房间参与者与连接状态。两者必须解耦：同一成员可以在 local 或 AI-AI 模式操作两色，AI controller 不是伪造的第二个成员或 token。
- 房主浏览器加载 KataGo，并继续使用**房主已有的房间身份和 token**控制被分配给它的 AI controller；不存在 AI proxy token、私有代理 session 或额外 WebSocket。轮到 AI 时 online adapter 发送明确的 `ai_play` 或 `ai_pass`，payload 带 `expectedMoveCount`、`expectedPositionToken` 和落点（若有）。
- DO 对 `ai_play/ai_pass` 依次验证：命令发送者是该颜色 controller 的 operator、模型/手数/positionToken 新鲜、当前确为对应 AI 回合、动作满足普通合法性和时钟规则。任一失败都不得修改棋局；显式 AI 命令是权限边界，不是绕开 GoEngine 的后门。
- v2 的 `friend`、`human-ai`、`ai-ai`、`local` 四种模式全部可在 online transport 中建立。`friend` 的黑白 controller 分属两名成员；其余三种模式由房主浏览器按 `match.controllers` 操作一个或两个颜色。AI controller 不占用成员席位，因此模式切换只在下一轮请求中替换 controller，不需要创建或释放虚假 AI 成员。
- 同一房主身份在新窗口连接时，新连接替代旧连接，旧连接停止重连。断线保留成员身份和本轮 `match.controllers`，也不暂停权威时钟；恢复后先 sync，旧 token 的推理结果一律丢弃。若浏览器未及时为其 AI controller 应手，该颜色按普通棋手规则超时。
- 旧房间中的 `automated` 白席及 `attach_ai/detach_ai` 仅用于 schema v1 兼容和迁移验证；新的 setup-first 房间、UI 和验收不得把它们当作 controller 的主要数据模型。
...
- 两种 transport 使用同一计时纯函数。`local` 由 match session 中的本标签页时钟裁决，AI 自对弈暂停时暂停本地时钟；`online` 只接受 DO 权威时钟。任一 AI controller 的模型加载、后台标签页或 operator 断线本身不暂停在线时钟。
...
- 本地或在线 `human/ai`、`ai/human`：人类可直接悔棋，不需要 AI 批准；撤回到人类上一次决策之前。在线 adapter 发送 `direct_undo_ai_round` 和 `expectedMoveCount + expectedPositionToken`，DO 确认发送者控制对应 AI controller 后原子撤回相应一轮并广播。
- 在线同机双人若黑白两个 human controller 的 `operatorId` 相同，则使用 `direct_undo_local_round` 直接撤回最近一手；DO 必须验证两色确由同一成员控制。不得为“自己请求、自己同意”创建伪协商流程。
...
- 在线真人 controller 在棋局尚未结束时可代表自己控制的颜色认输，不受当前轮次限制，也不需要对手同意；人机模式只有 human controller 可主动认输。服务端从已认证成员身份和 `match.controllers` 推导认输方，拒绝客户端伪造未受其控制的颜色，并立即广播一致终局。
...
| 单值长度 | 256 KiB |
| 属性 ID 长度 | 32 |
| 当前 SGF 导入维度 | 每轴 3–30（`maxBoardDimension=30`） |
...
Cloudflare Worker（静态资源、REST 路由、同源校验）
        ...
        │  BadukRoom（每个房间一个权威顺序执行器和持久状态）
        └─ optional binding: BADUK_ROOM_INDEX
           BadukLobby（由公开房间快照派生的可丢弃单向索引）
~~~
...
- 权威房间类名为 `BadukRoom`，SQLite 迁移标签为 `v1`；派生目录类名为 `BadukLobby`，由 `v2` 迁移加入。房间存储主键为 `room`，索引存储主键为 `rooms`，可观测性打开。
...
- 路由分工固定为：`/` 规范到 `/single`；`/single` 是零同步的本地 App；`/online/{code}` 是可直接分享和恢复的在线房间；实验性大厅使用独立、不在普通 UI 暴露的入口。旧 query/hash/`/room`/`/join` 链接可以兼容解析，但新分享链接必须生成 `/online/{code}`。
- 打开 `/single` 不访问房间 API，也不预建空 Durable Object。只有用户显式创建或加入在线房间后才建立服务器资源；单人页的设置、预览、AI 和本地棋局不依赖大厅索引。
- `BadukRoom` 是唯一房间事实源：先持久化房间，再用 `ctx.waitUntil` 尽力向可选 `BADUK_ROOM_INDEX` 发送公开快照。房间代码不得读取大厅、不得等待索引成功、不得借索引恢复状态。索引不存在、超时、损坏、清空或重新构建都不能影响房间创建、落子、计时、聊天、重连、TTL 或历史。
- `BadukLobby` 只从房间快照派生有限公开摘要并提供筛选/排序；不得保存私有 token、聊天正文、完整棋谱或可写房间句柄。它可以删除过期条目并整库重建，是“房间 → 大厅”的单向目录，而不是房间注册中心。
...
- 创建者获得真人黑方房主身份；第二位真人加入后获得白方成员身份，但仍处于房间设置/邀请阶段。AI 或同机模式不创建虚假成员，黑白 controller 可以同时由房主操作。两名真人成员到齐后后来者只能观战；加入 UI 必须把“加入对局”和“仅观战”作为两个明确动作，请求 `role:"spectator"` 时不得因有空席而自动改成玩家。
- 在线房间创建成功后生成带房间定位信息的可分享链接；链接不得包含任何 session token。好友模式必须另外保存带 revision 的 `match.request`，受邀白方明确接受后才进入 `playing`；拒绝、取消、过期或 revision 不匹配均保持/返回 `setup`，不能误入可落子状态。
...
- v2 AI controller 不创建成员、`playerId`、tokenHash、私有 session 或额外连接；其 `kind/operatorId/modelId` 只存在于 `match.controllers`，所有 AI 命令由 `operatorId` 对应的已认证成员使用原会话发送。schema v1 的 `role:"ai"`、`automated:true`、`controllerId` 只在兼容读取旧房时出现。
...
加入请求为 `{ "name": "玩家名", "role": "player" | "spectator" }`。成功响应包含 `code`、公开身份、私有 `token` 和当前房间快照；设置 controller 不签发新 token。仅正方形兼容响应额外包含 `size`。
...
- 公共 UI 及新请求允许宽、高 5–30。底层反序列化器继续接受 3–30，以兼容小尺寸测试棋盘与既有状态；公开创建界面仍以 5 为最小值。
...
任一在线 AI controller 应手时保持同一信封，改为 `action:"ai_play"`，payload 为 `{row,col,expectedMoveCount,expectedPositionToken}`；停着使用 `ai_pass` 并省略 row/col。
...
- 支持动作：`request_game`、`respond_game`、`cancel_game_request`、`play`、`pass`、`ai_play`、`ai_pass`、`direct_undo_ai_round`、`direct_undo_local_round`、`toggle_dead`、`finish_scoring`、`resume_play`、`request_undo`、`respond_undo`、`cancel_undo`、`resign`、`chat`、`new_game`（旧房兼容）、`leave`、`sync`，并保留早期房间的 `attach_ai/detach_ai` 兼容。前端统一 `undo` 意图按 controller/operator 归属路由为两种 direct 命令或真人协商命令。
...
- 重连、悔棋、新局、恢复行棋或任何 revision 跳变后，浏览器必须取消旧 `positionToken` 上的在线 AI 请求和延迟 UI 动作；operator 只能从最新 snapshot 为自己拥有的 AI controller 重新调度。
...
- 房间创建后固定处于 `setup`；加入成员只改变参与者列表，不代表接受邀请，也不允许第一手。好友模式必须经过 `request_game` 和受邀者显式 `respond_game(accept:true)`；`human-ai`、`ai-ai`、`local` 由房主确认设置后直接进入 `playing`。
- `match.controllers` 是本轮黑白控制权的唯一权威来源。服务端对每个 `play/pass/ai_play/ai_pass` 核对已认证发送者是否等于当前颜色的 `operatorId`，并核对 `kind`、轮次、坐标、合法性、拓扑、超级劫、阶段、手数和 positionToken；客户端预检只用于即时提示。
- 两次连续停着进入 `scoring`。获授权的 human operator 可以切换一整块死子；任何死子变更都清空确认。双方 human operator 分别确认；同一 operator 拥有两色或拥有人机中的 human controller 时，可按 controller 归属补齐所需确认。
- 在线不同 operator 的 `human/human` 使用 `request_undo/respond_undo/cancel_undo` 协商；同一 operator 拥有两个 human controller 时使用 `direct_undo_local_round` 撤回一手；拥有人类与 AI controller 时使用 `direct_undo_ai_round` 回到该人类上次决策点。三条路径均由 DO 原子修改 GoEngine、Replay、moveCount 和权威时钟。
- 认输不是协商操作：已认证 human operator 在未终局时可代表自己控制的颜色发起，不受轮次限制且无需对手确认。服务端拒绝伪造未受其控制的颜色；成功后立即停止时钟、清空悔棋申请与点目确认、持久化并广播。
- 每轮终局进入 `finished`；房主再次 `request_game` 时可沿用或调整设置，开始新轮前把上一轮完整 Replay、结果、设置、controller 和手数写入最多 24 轮的 `roundArchive`。成员、房间码、聊天和观众不因换轮丢失。
- 房间快照必须同时带 `revision`（兼容别名 `version`）、`positionToken`、`match`、`roundArchive`/公开 `rounds` 摘要、`moveCount`、`game`、`replay`、`timeControl`、`undoAvailable`、`undoRequest`、`scoreConfirmations`、公开 `players/spectators`、聊天摘要和到期时间。超时时 `game.phase="finished"`、`game.result`、`replay.outcome` 与 `timeControl.outcome` 必须一致；公开部分不得包含任何 token/hash。
...
interface OnlineRoomSnapshotV2 {
  ...
  match: {
    roundId: number;
    status: "setup" | "invited" | "playing" | "finished";
    mode: "friend" | "human-ai" | "ai-ai" | "local";
    controllers: Record<Color, {
      kind: "human" | "ai";
  ...
  roundArchive: ArchivedRound[];
  players: PublicMember[];
  spectators: PublicMember[];
}
~~~

`operatorId` 是控制归属而不是单独的凭据；DO 仍按当前已认证连接核对。任何 token/hash、命令限速桶和内部连接 ID 都必须从公开快照删除。旧 `players[].automated/controllerId` 只能作为 schema v1 迁移输入，不能覆盖 v2 `match.controllers`。
...
- `transport=online` 时同一组件绑定房间消息源；已认证真人成员可发送，AI controller 不能生成聊天，旁观者实时阅读但输入区禁用并说明“观战者只读”。

- 支持任意合法 Unicode 文字和普通 Emoji，不做敏感词、语义或立场审查，不自动改写、打码或屏蔽正文。
- 仍然执行长度、身份、频率、XSS 和协议校验；“不审文字”只表示不做内容层过滤。
- 只有已认证真人成员可以发送；AI controller 和旁观者不能发言。房主始终以自己的成员身份聊天，不能伪装成其操作的 AI controller 发言。
...
interface MatchSessionSnapshot {
  ...
  controllerByColor: Record<Color, "human" | "ai">;
  controllerOperatorByColor: Record<Color, string | null>; // online 来自 match.controllers
...
- controller 和 transport 仍是两个正交轴。online 可以保存便于 UI 选择的 `match.mode`，但权限与动作路由必须读取本轮 `match.controllers` 的 `kind/operatorId`，不得只凭 mode 或 members 猜测控制关系。
...
Durable Object 私有状态使用 `schemaVersion: 2`，至少保存：
...
interface StoredRoomV2 {
  ...
  roundArchive: ArchivedRound[]; // 最多 24 轮，含完整 replay
...
- schema v1 房间必须一次性迁移出 `match.controllers`、当前 lifecycle 和空 `roundArchive`；旧真人成员/automated 白席只用于确定迁移结果，迁移完成后 v2 权限不再从成员数组反向派生。
...
| `3d-baduk-locale-v1` | `zh-CN/en` | 用户设备长期偏好；合法值优先于浏览器语言 |
| `bamboo-baduk-player-name` | 最近使用的名字 | 用户设备长期偏好 |
| `bamboo-baduk.session.{ROOM}` | 本人 token、身份、`nextSequence`；同一 operator 控制多个颜色仍只保存这一份 session | 离开/过期前 |
...
侧栏参考 OGS 的信息密度而不复制品牌：黑白棋手/时钟卡始终位于标签栏上方；下方固定五个标签“棋局 / 分析 / 聊天 / 棋谱 / 棋盘设置”。五个标签及其面板在 local/online、当前支持的 controller 组合和观战身份下必须是**同一组 DOM 节点**，只根据 match session snapshot 改内容、权限、disabled/hidden 子控件和提示；不得销毁本地侧栏再创建联机侧栏。一次只显示一个标签面板，隐藏面板不得占布局或进入键盘 Tab 顺序；标签切换不修改棋局、复盘光标、AI 请求或房间状态。

“棋盘设置”尤其只能有一份表单 DOM：拓扑、宽高、规则、贴目和计时都从同一表单读写。它在 `setup` 中是下一局的可编辑草稿并实时驱动预览；进入 `invited/playing` 后按身份锁定，不能通过修改字段或顶部拓扑按钮绕过邀请流程重开。进入 `finished` 后，本地设备或在线黑方房主点击“调整下一局设置”，或者直接打开“棋盘设置”，都必须进入同一个预填设置确认阶段并重新开放表单；共享卡片使用“设置并发起下一局 / 使用当前设置开始下一局”，在线白方和观众仍只读。controller/transport 在同一邀请流程的最后一步选择；切换选项不得丢失草稿、重复 ID/事件监听或造成两个互相不同步的隐藏表单。

初始大厅沿用同一页面骨架，但侧栏首要卡片显示“我的房间 / My Room”和一个占据主要可用宽度的“邀请对局 / Invite Game”按钮；“加入邀请/输入房间码”是次要动作。棋盘舞台可用覆盖层再次提供同一个邀请动作及当前设置摘要，两个入口打开同一个弹层，不得维护两份表单。大厅不显示“黑方落子”“对局中”或正在走动的时钟；尚无意义的棋局操作应隐藏或禁用。进入 `playing` 后才把覆盖层撤下，并用同一侧栏节点显示棋手、时钟和棋局操作。
...
- 终局的“直接进行下一局”只允许使用完整、未经修改的上一局配置并立即开局；“调整下一局设置”必须先切到棋盘设置并展示预填预览，同一设置区内提供“使用当前设置开始下一局”，在用户确认前不得重置权威棋局、启动 AI、启动计时或发送在线 `new_game`。
- match lifecycle 进入 `playing` 后，侧栏自动切回“棋局”，使停一手、悔棋和认输控件立即可见；创建/加入仍处于 `setup/invited` 的在线房间时默认打开“棋盘设置”或邀请状态，不得提前显示可用棋局动作。之后可自由切换五个固定标签。
- 上一条中的棋局操作只在 match lifecycle 真正进入 `playing` 后可用；仅创建好友邀请但仍等待白方时，侧栏显示邀请链接和等待状态，棋盘保持可观察但不可落子，权威计时也不得提前启动。
...
- “聊天”标签始终存在；local 显示联机引导，online 真人成员显示输入，存在 AI controller 时说明 AI 不会聊天，旁观者显示只读历史。
...
- 产品必须完整提供简体中文 `zh-CN` 与英文 `en`。首次访问且没有合法持久偏好时，按 `navigator.languages`/`navigator.language` 中第一个有效首选语言选择：以 `zh` 开头则使用简体中文，其他语言回退英文。
- 页面顶栏必须有始终可见、可键盘操作的语言切换控件。选择写入 `3d-baduk-locale-v1`；以后刷新优先使用该合法持久值，不因浏览器语言变化而覆盖。存储不可用或值损坏时安全回退到浏览器语言规则。
- 切换语言必须即时更新 `<html lang>`、静态文案、按钮、字段标签、placeholder、`title`、`aria-label`、状态/错误、AI 模型与资源说明、计时/复盘/房间/聊天系统文案，以及 Canvas/WebGL 中由产品绘制的文字；不刷新页面、不重建棋局、不重置复盘光标或相机。
- 玩家名、聊天正文、房间码、SGF 注释和其他用户原文必须原样显示，不得机器翻译。语言只属于当前浏览器的本地偏好，不进入房间 snapshot、WebSocket 命令或 SGF，也不改变对手/观战者页面。
- 所有新增功能必须同时提供两种语言，不能依赖浏览器自动翻译；静态与动态文案都要有自动化覆盖，避免参数变化后漏翻或误翻用户原文。
...
- `modelId`、`operatorId/controllerId`、`expectedMoveCount`、`expectedPositionToken` 和“我是 AI”声明同样不可信。服务端只允许已认证成员操作 `match.controllers` 中归属于自己的颜色，并从当前状态重新计算 positionToken；该 token 只防异步陈旧结果，不能代替会话鉴权或命令幂等 ID。
...
| 客户端 WS 消息 | 8 KiB |
| 连接 | 64/房间 |
| AI controller | 每轮最多黑白各 1 个；都无独立成员、连接或 session，可由同一 operator 串行驱动 |
| 观战者 | 32/房间 |
| 未连接观战预约 | 60 秒 |
| 已连接观战者断线宽限 | 5 分钟 |
...
3. 输入小于 5、大于 30、空值、小数、`NaN/Infinity` 时按第 3.1 节确定处理，服务端不能创建越界新局。
4. 对 7×11、11×7、5×30、30×5 和 30×20 棋盘，交叉点总数、board 形状、落子、聊天坐标、SGF 和 AI 索引均为 `W × H`，无裁切、错列或重复点。
...
8. setup 预览中切换拓扑会立即重建空白预览并切到该形状最直观的视图；`playing` 中拓扑和宽高全部锁定。终局点击“直接进行下一局”会按完整旧配置提交同一新轮请求（好友模式仍待对方接受）；点击“调整下一局设置”则显示预填上一局全部参数的空白预览，确认前不建立新局，修改为新拓扑或尺寸并确认后必须按新邻接建立全新棋局。
...
6. 点目确认由 match session 按 controller/operator 收集：不同 operator 的在线真人双方各自确认；同一 operator 控制两色或人机时按其 controller 归属补齐；任一死子变更清空确认。local 各组合使用同一评分结果。
7. 同设备悔棋正好回退一事件并恢复完整历史；online 同机双人通过 `direct_undo_local_round` 原子回退一手；人机“直接悔棋”在 local/online 都回到人类上次决策点；只有不同 operator 的在线真人对真人显示“申请悔棋”，未经对方同意绝不回退。
...
1. transport 与 controllerByColor 是两个独立轴：local 的四种组合，以及 online 的 `friend/human-ai/ai-ai/local` 四种持久模式，都使用一个 MatchSession、同一动作 reducer 和同一五标签/设置 DOM；不存在按模式复制的互斥 UI。
2. 切换 transport 或用 `request_game` 建立不同 controller 的下一轮时，旧 AI、WebSocket/timer 和监听被取消，但侧栏标签、设置节点、表单值与无关视图偏好不被复制或丢失；重复切换 100 次仍一次点击只触发一个动作。`attach_ai/detach_ai` 只测旧房兼容，不作为 v2 UI 流程。
...
6. AI 自对弈在 local 和 online 都只创建一个模型实例/Worker，黑白串行复用，默认两手间约 420 ms；暂停后无新手，恢复后继续，只有暂停时可悔一步。online 每手仍须经 DO 验证、持久化和广播。
7. 在线任一 AI controller 都由其 `operatorId` 成员使用原会话发送 `ai_play/ai_pass`；命令带 expectedMoveCount/expectedPositionToken，伪造 operator、旧位置和错误回合均被拒绝，玩家与观众只看到一次权威落子，`players` 不出现虚假 AI 成员。
...
12. 每个在线权威 snapshot 有 positionToken；用上一个 token/手数重放 `ai_play/ai_pass`、`direct_undo_ai_round` 或 `direct_undo_local_round` 返回 `STALE_GAME_STATE`，棋盘、时钟和 replay 不发生业务变化。
13. 新建在线房固定进入 `setup`。`friend` 请求进入 `invited` 且加入成员不等于接受；明确接受后才 `playing`。`human-ai/ai-ai/local` 由房主确认后直接 `playing`，四种模式的 controller 都持久化并在刷新后恢复，无第二 proxy session。
14. 两色 AI 应手、两种直接悔棋、真人协商悔棋、点目、认输和观战在玩家/观众端一致，服务端进程中没有模型或推理任务；结束一轮后完整归档，连续建立超过 24 轮只保留最近 24 轮。
...
6. 桌面与移动端都能在“棋局/分析/聊天/棋谱/棋盘设置”间切换，黑白时钟不因切标签消失；隐藏标签内容不进入 Tab 顺序，候选 hover 与键盘 focus 行为一致。

### 19.9 大厅、邀请与本地化

1. 清除站点偏好后，以中文浏览器打开显示简体中文，以英文或其他语言浏览器打开显示英文；手动切换后刷新仍保持所选语言，`<html lang>` 与可访问名称同步变化。
2. 首次进入只显示“我的房间 / My Room”和醒目的“邀请对局 / Invite Game”，不得出现“开始对局 / Start Game”作为该主要动作；棋盘为空且没有“黑方落子/对局中”假状态。
3. 在大厅旋转、缩放、平移和切换视图均可用；左键/轻点任意棋点都不落子，不产生手数、Replay、声音或计时变化，也不显示幽灵棋。
4. 初始加载及取消邀请后，房间创建 API、WebSocket、AI 模型/Worker、自动行棋和棋钟均未启动；重复打开/关闭邀请弹层 20 次仍没有资源泄漏或网络副作用。
5. 同一个邀请弹层可选择本地双人、本地 AI、AI 自对弈和好友在线，并复用设置区的同一份拓扑、矩形宽高、规则、贴目、计时和 controller 校验；切换选项不生成第二组隐藏表单。
6. 确认三种本地配置后才进入 `playing` 并解锁对应棋局操作；创建在线房后先进入 `setup`。房主选择 `friend/human-ai/ai-ai/local` 并发送一次 `request_game`：后三种直接开局，friend 只进入 `invited`，此时仍不能落子或启动时钟。
7. 第二浏览器打开邀请链接并成功以玩家加入后仍停留在设置/邀请状态；只有受邀者显式点击接受并成功发送 `respond_game(accept:true)`，两个客户端才进入同一权威棋局且第一手/时钟可用。拒绝、取消、过期、无效、revision 不匹配或已满时均不能误进入 playing。
8. 从 local 大厅到本地棋局、退出后邀请好友、再返回 local，始终只有一套棋盘、五标签、设置 DOM 和动作处理器；无重复 ID，单次点击只触发一个业务动作。
9. 对局中切换语言不会改变棋盘、手数、当前方、计时、AI 任务、房间 revision、复盘帧或视角；另一名玩家/观战者的语言不随之改变。玩家名、聊天与 SGF 原文不被翻译。
...
3. **房间集成测试**：内存 RoomEngine + 假 WebSocket/fetch，覆盖 setup/invited/playing/finished、四种 online mode、持久 `match.controllers`、request/accept/reject/cancel、`ai_play/ai_pass`、两种直接悔棋与真人协商悔棋、24 轮归档、手数/token stale 拒绝、幂等、重连、点目、聊天、权威超时、alarm、TTL、schema v1 兼容和回滚恢复。
4. **浏览器交互测试**：浏览器语言选择/持久切换、大厅零副作用、统一设置入口、好友加入后仍需显式接受、实际开局前禁用棋局动作，以及真实 Canvas pointer/touch、拖动阈值、resize、侧栏/时钟、transport × 全部 controller 组合、唯一 DOM、聊天本地引导、房主浏览器调度 AI controller、观战本地分析、候选 hover/focus/pin、视图切换和 reduced motion。现有 Node 测试不能替代这一层。
5. **线上 smoke**：对已部署 Worker 分别建立 `friend`、`human-ai`、`ai-ai`、`local` 房；连接玩家/观战端，验证 setup-first 邀请、原 operator 会话的 AI 命令、positionToken/手数、落子、两种直接与协商悔棋、计时快照/超时、点目、聊天、pass/状态一致、多轮归档、断线恢复和离开。
...
| --- | --- | --- | --- | --- |
| 5×5、9×9、13×13、19×19、30×30 | 三种 | 全部 | 每拓扑全部可用视图 | b10 全部；大盘允许更慢 |
| 7×11、11×7 | 三种 | 全部 | 每拓扑全部可用视图 | b10 全部，b18 抽样 |
| 5×30、30×5、30×20 | 三种 | 邻接/提子/点目/SGF | Flat + 各自 3D | padding/合法性/索引 |

两个轴的组合覆盖（`H/AI` 同时覆盖人类执黑与执白）：

| 场景 | local H/H | local H/AI 或 AI/H | local AI/AI | online friend | online human-ai | online ai-ai | online local | online 观战 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 权威动作路径 | local adapter | local adapter | local adapter | DO command | operator 原会话 + AI command | operator 原会话 + AI command | DO command | 只读 snapshot |
| 落子/提子/pass/结束 | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 只读观察 |
| 悔棋 | 直接一手 | 人类决策点 | 暂停后一步 | 真人协商 | `direct_undo_ai_round` | 暂停后 DO 撤回 | `direct_undo_local_round` | 禁止 |
| 点目确认 | 同设备补齐 | 人类直接 | 暂停后人工 | 双方确认 | human operator 补齐 | 暂停后人工 | 同 operator 补齐 | 只读 |
| 认输 | 当前人类颜色 | 始终人类方 | 禁止 | 任一真人 controller | human controller | 禁止 | 当前人类颜色 | 禁止 |
| 复盘/切视图/SGF | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | 公开 replay |
| 聊天标签 | 联机引导 | 联机引导 | 联机引导 | 真人可发 | operator 可发、AI 不发 | operator 可发、AI 不发 | operator 可发 | 只读 |
| 主时间/读秒 | 本地权威 | 本地权威 | 暂停联动 | DO 权威 | DO 权威 | DO 权威 | DO 权威 | 只读显示 |
| 实时 AI 局势分析 | ✓ | ✓ | 暂停后 | 进行中棋手禁用 | 进行中 operator 禁用 | 运行中禁用 | 进行中禁用 | 本地可用且不广播 |
| 五标签和设置 DOM | 同一组 | 同一组 | 同一组 | 同一组 | 同一组 | 同一组 | 同一组 | 同一组只读/按权限提示 |
| 终局新局操作 | 同配置或预填确认 | 保留模型/执子 | 保留自对弈 | 房主 request_game | 保留模型 | 保留自对弈 | 保留同机双人 | 等待房主、只读 |
...
- `matchSession.test.js`、`aiMatchMode.test.js`、`onlineRoundEngine.test.js`：两个正交轴、local 与 online 全模式矩阵、持久 controller/operator、setup-first 邀请、统一动作路由、positionToken、单 Worker 自对弈暂停/恢复/悔棋/双 pass 和 24 轮归档。
- `gameSounds.test.js`、`movePreviewPolicy.test.js`：声音恰一次和幽灵棋策略。
- `protocol.test.js`、`commandSync.test.js`、`roomClient.test.js`、`roomEngine.test.js`：协议、幂等、positionToken/expectedMoveCount、`match.controllers` 权限、`request_game/respond_game/cancel_game_request`、`ai_play/ai_pass`、`direct_undo_ai_round/direct_undo_local_round`、重连、协商悔棋、显式观战权限、服务端计时、alarm、TTL 和 schema v1 回滚兼容。
...
必须补充真实浏览器测试文件或等价方案，专门回归两组缺口：其一是主指针、左键落子、左键拖动不落子也不浏览、右键平移/旋转但不落子、右键菜单抑制、6 px 阈值、`pointercancel`、触摸滚动冲突、Canvas resize 命中、弧面滑动和坐标按钮聚焦；其二是切换 local/online 与四种 online mode 后五标签和设置仍为同一 DOM 节点、监听不重复、local 聊天显示联机提示、operator 只在新 positionToken/手数上调度自己拥有的 AI controller，且两种直接悔棋/协商悔棋/点目/时钟均由当前 adapter 确认。不能用更多纯函数测试声称已解决。
...
- schema v1 房间必须迁移到 schema v2：从旧真人/automated 成员确定初始 `match.controllers`，根据棋局终局状态设置 `playing/finished`，并初始化空 `roundArchive`；未知更高 schema 仍拒绝。后续破坏性变化必须继续升 schema，不得原地改变含义。
...
- schema v1 自动白席通过成员上的可缺省 `automated/modelId/controllerId` 持久化；迁移器只读这些字段并生成 v2 `match.controllers`，不签发 proxy 凭据。v2 新房不再写虚假 AI 成员。回滚到仅认识 v1 的版本时，必须明确披露其不能理解 v2 setup-first、多 controller 和多轮归档，不能假装在线 AI 仍正常运行。
- 重新升级时以已经持久化的 v2 `match.controllers` 为主；只有尚未迁移的 v1 数据才从成员派生一次。若旧 automation 数据缺少有效真人 operator，则任何成员都无权发送 AI 命令；不得猜测或自动转移控制权。
...
6. 实现 online transport adapter、RoomEngine/协议/RoomClient/Cloudflare Durable Object；以 schema v2 `match.controllers` 建立 setup/invited/playing/finished、`friend/human-ai/ai-ai/local`、显式 request/accept/reject/cancel、`ai_play/ai_pass`、两种直接悔棋、真人协商悔棋、点目与最多 24 轮归档。`attach_ai/detach_ai` 只放在 v1 兼容层。
7. 在唯一五标签/设置 DOM 中接入所有 transport/controller 状态；在权威聊天状态上实现文字、表情包、坐标、限速和观战只读，local 聊天显示联机引导；在本地副本上实现观战分析并证明零广播。
8. 完成 controller/transport 切换资源清理、schema v1 迁移、浏览器端到端，以及四种 online mode 的玩家/观战 smoke；验证加入不等于接受、AI-AI 串行调度、local 双 controller 和 24 轮归档。
...
- 矩形、SGF（含超时与认输结果）、b10/b18、local 四种 controller 组合、online `friend/human-ai/ai-ai/local`、显式邀请接受/拒绝、持久 `match.controllers`、授权 AI 命令/positionToken、24 轮归档、多候选 PV、主时间/日式读秒、显式观战、本地观战分析、两种直接与真人协商悔棋、认输、点目和聊天文字/表情/坐标在真实部署可用。
